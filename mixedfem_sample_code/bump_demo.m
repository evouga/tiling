...
if(strcmp(mesh_type,'XXXX'))
    mesh = readOFF('both_slices.off');
    V = mesh.V;
    F = mesh.F;
else
...
inout = textread('both_slices_orig.txt', '%d');
...
if(strcmp(mesh_type,'XXXX'))
    %zlimit = 1.15;
    %interior = indices(V(:,3) < zlimit & V(:,3) > -0.25);
    %exterior = indices(V(:,3) >= zlimit | V(:,3) <= -0.25);
    bot = V(:,3) < 0;
    botind = bot & (inout == 1);
    meanx = mean(V(:, 1));
    meany = mean(V(:, 2));
    ...
    %V(:,1) = V(:,1) + (V(:,1) - meanx) .* (maxz - V(:,3)) * 20;
    %V(:,2) = V(:,2) + (V(:,2) - meany) .* (maxz - V(:,3)) * 20;
    %goodV = indices(V(:,3) > min(V(:,3)) + 0.01);
    %bad = indices(V(:,3) <= min(V(:,3)) + 0.01);
    %goodF = find(~ismember(F(:,1), bad) & ~ismember(F(:,2), bad) & ~ismember(F(:,3), bad));
    %F = F(goodF,:);
    %V = V(goodV,:);
    %%%V(botind,2) = V(botind,2) + (meany - V(botind,2)) .* 2;
    %inout = inout(goodV);
    %inout(V(:,3) > 0 & V(:,3) < 1) = 1;
    interior = indices(inout == 1);
    exterior = indices(inout == 2);
end
